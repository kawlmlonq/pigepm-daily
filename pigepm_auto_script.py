    ...
    for attempt in range(max_retries):
        try:
            ...
            if worksheet.cell(1, 1).value != "日期時間":
                print("📝 建立標題列")
                worksheet.update('A1:D1', [["日期時間", "牧場數量", "使用者數量", "數據來源"]])
                
                # 格式化標題列（粗體、背景色）
                fmt = cellFormat(
                    backgroundColor=color(0.9, 0.9, 0.9),  # 淺灰色背景
            ...
            format_cell_range(worksheet, 'B:C', num_fmt)
...
if __name__ == "__main__":
    try:
        print("🚀 程式啟動中")
        print(f"⏰ 執行時間：{(datetime.now(timezone.utc) + timedelta(hours=8)).strftime('%Y/%m/%d %H:%M:%S')} (台灣時間)")
